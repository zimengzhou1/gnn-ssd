...
std::tuple<torch::Tensor, int, int> gather_ginex(std::string feature_file, torch::Tensor idx, int64_t feature_dim, torch::Tensor cache, torch::Tensor cache_table){
    ...
    return std::make_tuple(result, hits, misses);
